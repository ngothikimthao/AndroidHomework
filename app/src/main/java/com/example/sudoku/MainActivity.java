...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for( int i=0;i<buttons.size();i++){
            setValue(buttons.get(i),"a");
        }

        for(int i=1;i<buttonEnterNumber.size();i++){
            numberChoose = getValue(buttonEnterNumber.get(i));
        }
    }

    String getValue(final Button button){
        final String[] value = {""};
        button.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                value[0] = button.getText().toString();
            }
        });
        return value[0];
    }

    void setValue(final Button button, final String text){
